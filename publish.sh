...
#   security add-generic-password -a "bio-startup-space" -s "google-sa-datasets-498814" -w "$(cat ~/Downloads/your-key.json)"
#
...
GOOGLE_SERVICE_ACCOUNT_JSON="$(security find-generic-password -a "bio-startup-space" -s "google-sa-datasets-498814" -w 2>/dev/null | xxd -r -p || true)"

if [[ -z "$GOOGLE_SERVICE_ACCOUNT_JSON" ]]; then
  echo "Error: key not found in Keychain." >&2
  echo "Store it with:" >&2
  echo '  security add-generic-password -a "bio-startup-space" -s "google-sa-datasets-498814" -w "$(cat ~/Downloads/your-key.json)"' >&2
  exit 1
fi
